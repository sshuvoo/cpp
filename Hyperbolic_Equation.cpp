// Finite difference scheme to solve a 1D wave equation with two point bc's
#include <cmath>
#include <iomanip>
#include <iostream>
using namespace std;
...
int main() {
    ...
    for (i = 0; i <= n; i++) {
        u[i][1] = ((pow(lam, 2)) * (u[i + 1][0] + u[i - 1][0])) / 2 +
                  (1 - pow(lam, 2)) * u[i][0] + k * 0;
    }

    // Values for other grid points
    for (j = 1; j <= mm; j++) {
        for (i = 1; i < n; i++) {
            u[i][j + 1] = (pow(lam, 2)) * (u[i + 1][j] + u[i - 1][j]) +
                          2 * (1 - pow(lam, 2)) * u[i][j] - u[i][j - 1];
        }
    }
